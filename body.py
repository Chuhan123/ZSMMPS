...
mask_output_dir = "body_masks"  # 修改为point_masks
prompted_output_dir = "seg_body"
colored_output_dir = "colored_bodyseg"  # 新增彩色分割输出目录
model_type = "vit_h"  
...
def process_model(model_name):
    ...
    for view_name in view_names:
        ...
        # 定义身体分割规则（明确设置：正标签3，负标签1）
        parts = [
            {'name': 'body_part', 'positive': [1], 'negative': [3], 'color': (0, 255, 255)},  # 黄色-body
        ]
        ...
        if 'body_part' in part_masks:
            colored_seg[part_masks['body_part']] = parts[0]['color']  # 黄色掩码
        ...
        # 保存语义掩码
        semantic_masks = {
            'body_part': part_masks.get('body_part', np.zeros_like(mask) if 'mask' in locals() else None),
        }
